...
<head>
	...
	<meta charset="utf-8">
        ...
        <meta category="description" content="">
        <meta category="author" content="">
...
<body>
        ...
        <div class="container-fluid">
            <div id="pad-wrapper">
                ...
                <div class="row-fluid section">
                    <h4 class="title">User Consumption By Category</h4>
    ...
    <script type="text/javascript">
        ...
        Morris.Bar({
	        ...
	        {
				double sells = data.get(category).doubleValue();
            %>
                {category: '<%=category %>', sells: <%=df.format(sells) %>},
           	<%
	        }
            }
            %>
        ...
        Morris.Donut({
            ...
            for(String category : data.keySet())
	        {
				double sells = data.get(category).doubleValue();
            %>
                {label: '<%=category%>', value: <%=df.format(sells)%> },
            <%
	        }
            }
            %>
     ...
     // Morris Line Chart
		...
		for(Object object : dataYear)
		{
			...
			if (row.get("year") instanceof Integer){
			    ...
			    sells = (Double)row.get("sells");
			}
		%>
        	{"period": "<%=year%>", "sells": <%=df.format(sells)%>},
        <%
		}
        %>
        ];
		...
		%>
			{"period": "<%=year%>-<%=month%>", "sells": <%=df.format(sells)%>},
		<%
		}
		%>
        ];
		...
		%>
			{"period": "<%=year%>-<%=month%>-<%=day%>", "sells": <%=df.format(sells)%>},
		<%
		}
		%>
        ];   
